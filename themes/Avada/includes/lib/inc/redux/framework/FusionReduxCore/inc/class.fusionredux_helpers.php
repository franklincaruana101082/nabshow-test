	...
	class FusionRedux_Helpers {
		...
		public static function getTrackingObject() {
			...
			$data = [
				...
				'options'     => apply_filters( 'fusionredux/tracking/options', [] ),
				'theme'       => $theme,
				'fusionredux' => [
				...
				'developer'   => apply_filters( 'fusionredux/tracking/developer', [] ),
				'plugins'     => $plugins,
			];
		...
		public static function localize( $localize ) {
			...
			$localize['rAds'] = self::rURL_fix( $base, $fusionredux->args['opt_name'] );
		...
		private static function getFusionReduxTemplates( $custom_template_path ) {
			...
			$template_paths     = [ 'FusionReduxFramework' => FusionReduxFramework::$_dir . 'templates/panel' ];
			$scanned_files      = [];
			$found_files        = [];
				...
				foreach ( $files as $file ) {
					...
					if ( $theme_file ) {
						...
						if ( $core_version && ( empty( $theme_version ) || version_compare( $theme_version, $core_version, '<' ) ) ) {
							...
							$found_files[ $plugin_name ][] = sprintf( __( '<code>%1$s</code> version <strong style="color:red">%2$s</strong> is out of date. The core version is %3$s', 'Avada' ), str_replace( WP_CONTENT_DIR . '/themes/', '', $theme_file ), $theme_version ? $theme_version : '-', $core_version );
						} else {
