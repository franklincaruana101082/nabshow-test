...
class WP_Batch_Processor_Admin {
	...
	public function admin_menu() {
		...
		add_submenu_page(
			...
			__( 'Import Companies', 'wp-batch-processing' ),
			__( 'Import Companies', 'wp-batch-processing' ),
			'manage_options',
	...
	public function plugin_page() {
		$action = 'batch-view';
		$id     = 'nab_import_companies';
	
		WP_BP_Helper::render( $action, array( 'id' => $id ) );
	}
	...
	private function is_batch_runner_screen( $action = null ) {
		$is_main_screen = isset( $_GET['page'] ) && $_GET['page'] === 'amplify_company_import';
		if ( ! is_null( $action ) ) {
