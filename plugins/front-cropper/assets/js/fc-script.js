(function ($) {
  ...
  $(document).ready(function () {
    ...
    $(document).on("change", ".cropper_img_file", function (e) {
      ...
      let ratio =
        undefined !== requestedBy && -1 !== requestedBy.indexOf("banner")
          ? 4
          : 1;
      ...
      if (0 === $("#cropper-board").length) {
        ...
        cropperCancel.setAttribute(
          "class",
          "cropper-cancel bm-btn nab-modal-remove"
        );
      ...
      if (cropper_input_file.files && cropper_input_file.files[0]) {
        ...
        reader.onload = function (e) {
          ...
          if (
            "profile_picture" === requestedBy ||
            "banner_image" === requestedBy
          ) {
    ...
    $(document).on("click", "#cropper-button", function () {
      if ("" !== window.cropper && undefined !== window.cropper) {
        ...
        if (0 !== $(".bm-crop-upload").length) {
          const croppedImg = window.cropper
            .getCroppedCanvas()
            .toDataURL("image/png");

          $(".bm-show-cropped-thumb").html(
            '<div id="cropped-view"><img src="' +
              croppedImg +
              '" id="bm-copped-display" /><div class="bm-btn-wrap"><a href="javascript:void(0)" class="nab-modal-close bm-btn-pink">remove</a></div></div>'
          );
        } else {
