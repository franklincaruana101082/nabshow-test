jQuery(document).ready(function($) {
    ...
    expandAllLink.setAttribute('class', 'acs-expand-all');
    expandAllLink.setAttribute('href', 'javascript:void(0);');
    expandAllLink.innerText = acs_l10n_vars.lexpandall;

    var collapseAllLink   = document.createElement('a');
    collapseAllLink.setAttribute('class', 'acs-collapse-all');
    collapseAllLink.setAttribute('href', 'javascript:void(0);');
    collapseAllLink.innerText = acs_l10n_vars.lcollapseall;

    /*
     * Add Expand/Collapse ALL Links to DOM (has to be first for listeners)
     */
    if ( 0 < jQuery('.acs-hier .subsubsub').length ) {
    ...
    if ( 0 < jQuery('.acs-hier.taxonomy-category .actions').length ) {
