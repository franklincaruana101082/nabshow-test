	...
	class CTCC_Metafield {
		...
		*/
		...
		public function metabox_checkbox_output( $post, $field ) {
			...
			?>
            <div class="dm-work-metafield <?php echo esc_attr( $field['class'] ); ?>">

                <input type="checkbox" id="<?php echo esc_attr( $field['name'] ); ?>" name="<?php echo esc_attr( $field['name'] ); ?>" value="1" <?php checked( 1, $field_value ); ?>>
		...
		public function save_metabox_data( $post_id ) {
			...
			// Verify the nonce
			if ( ! isset ( $_POST['ctcc_metabox_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['ctcc_metabox_nonce'] ), 'save_metabox_data' ) ) {
				return;
			}
