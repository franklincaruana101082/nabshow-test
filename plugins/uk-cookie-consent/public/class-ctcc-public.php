	...
	class CTCC_Public {
		...
		public function add_css() {
			...
			if ( ! empty( $exclude ) ) {
                ...
                <style id="ctcc-css" type="text/css" media="screen">';
                    ...
                    <?php echo esc_attr($button_style); ?>
                    }
			...
			if ( ! empty( $exclude ) ) {
                ...
                <script type="text/javascript">
                    jQuery(document).ready(function ($) {
						<?php
						$cookie_value = filter_input( INPUT_GET, 'cookie', FILTER_SANITIZE_STRING );
						if ( isset ( $cookie_value ) ) { ?>
                        catapultDeleteCookie('catAccCookies');
						<?php } ?>
                        if (!catapultReadCookie("catAccCookies")) { // If the cookie has not been set then show the bar
                            $("html").addClass("has-cookie-bar");
                            $("html").addClass("cookie-bar-<?php echo esc_attr( $ctcc_styles_settings['position'] ); ?>");
                            $("html").addClass("cookie-bar-<?php echo esc_attr( $type ); ?>");
                        ...
                        setTimeout(ctccCloseNotification, <?php echo esc_html( $duration ); ?>);
                        ...
                        $(window).scroll(function () {
                            ...
                            if (scroll > <?php echo esc_html( $height ); ?> ) {
		...
		public function add_notification_bar() {
			...
			if ( ! empty( $exclude ) ) {
				...
				?>
                <div id="catapult-cookie-bar" class="<?php echo esc_attr( $classes ); ?>">

					<?php
					// Add a custom wrapper class if specified
					if ( $ctcc_styles_settings['position'] === 'top-bar' || $ctcc_styles_settings['position'] === 'bottom-bar' ) { ?>
                    <div class="ctcc-inner <?php echo esc_attr( str_replace( '.', '', $ctcc_styles_settings['container_class'] ) ); ?>">
						<?php }
						...
						if ( empty ( $ctcc_styles_settings['x_close'] ) ) {
							...
							$button_text = sprintf(
								'<button id="catapultCookie" tabindex=0>%s</button>',
								$accept_text
							);
						}
