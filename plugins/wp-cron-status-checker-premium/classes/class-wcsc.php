...
class WCSC {
    ...
    private static function _log( $hook_name, $key = 'start', $with_time = true, $result = null ) {
        ...
        if ( $key == 'start' ) {
            ...
            $params = array( 'end' => $with_time ? microtime( true ) * WCSC::MICROTIME_TO_INT_MULTIPLE : 0 );
            $formats = array( '%d' );
            if ( !is_null( $result ) ) {
                $params['result'] = $result;
                $formats[] = '%s';
        ...
        if ( $key !== 'start' && $log_lifespan === 0 ) {
            ...
            $wpdb->delete( WCSC::table_name(), array( 'id' => $log_id ) );
        }
    }
    ...
    private function summarize_log( $log, $hook_name = '' ) {
        ...
        if ( wcsc_fs()->is__premium_only() ) {
            if ( WCSC::is_result_error( $result_error_param ) ) {
    ...
    public static function cleanup() {
        ...
        if ( wcsc_fs()->is__premium_only() ) {
            if ( intval( $log_lifespan ) >= 604800 ) {
                ...
                if ( !empty( $to_delete ) ) {
                    $wpdb->query( "delete from " . WCSC::table_name() . " where hook_name IN ('" . implode("', '", $to_delete ) . "')" );
                }
            }
        }
    ...
    public static function humanize_interval( $interval ) {
        ...
        if ( $interval < 60 ) {
            return sprintf( _n( 'Every %s', 'Every %s', $interval, 'wcsc' ), self::humanize_seconds( $interval ) );
        }
        else if ( $interval < 3600 ) {
            return sprintf( _n( 'Every %s', 'Every %s', $interval / 60, 'wcsc' ), self::humanize_seconds( $interval ) );
        }
        else if ( $interval < 86400 ) {
            return sprintf( _n( '%s', 'Every %s', $interval / 60 / 60, 'wcsc' ), self::humanize_seconds( $interval ) );
        }
        else {
            return sprintf( _n( 'Once Daily', 'Every %s', $interval / 60 / 60 / 24, 'wcsc' ), self::humanize_seconds( $interval ) );
        }
    }
    ...
    public static function humanize_seconds( $number ) {
        ...
        if ( $number < 60 ) {
            return sprintf( _n( '%s second', '%s seconds', $number, 'wcsc' ), $number );
        }
        else if ( $number < 3600 ) {
            $number = $number / 60;
            return sprintf( _n( '%s minute', '%s minutes', $number, 'wcsc' ), $number );
        }
        else if ( $number < 86400 ) {
            $number = $number / 60 / 60;
            return sprintf( _n( 'hour', '%s hours', $number, 'wcsc' ), $number );
        }
        else {
            $number = $number / 60 / 60 / 24;
            return sprintf( _n( 'day', '%s days', $number, 'wcsc' ), $number );
        }
    }
