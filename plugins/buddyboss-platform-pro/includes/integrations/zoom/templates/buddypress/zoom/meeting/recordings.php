...
if ( ! empty( $id ) ) {
	bp_zoom_meeting_update_meta( $id, 'zoom_recording_count', count( $recordings ) );
}
...
<div id="bp-zoom-block-show-recordings-popup-<?php echo $meeting_id; ?>"
    ...
    <header class="bb-zm-model-header"><span class="bp-meeting-title-recording-popup"><?php echo $title; ?></span><?php _e( ' (Recordings)', 'buddyboss-pro' ); ?></header>
    ...
    <div class="recording-list-row-wrap">
		...
		foreach ( $recordings_groups as $date => $recording_group ) {
			...
			?>
            <div class="recording-list-row-group">
