...
function mo_openid_process_custom_app_callback() {
	...
	} elseif ( strpos( $_SERVER['REQUEST_URI'], 'openidcallback' ) !== false ) {
		if ( ( strpos( $_SERVER['REQUEST_URI'], 'openidcallback/google' ) !== false ) || ( strpos( $_SERVER['REQUEST_URI'], 'openidcallback=google' ) !== false ) ) {
			$appname = 'google';
		} elseif ( ( strpos( $_SERVER['REQUEST_URI'], 'openidcallback/facebook' ) !== false ) || ( strpos( $_SERVER['REQUEST_URI'], 'openidcallback=facebook' ) !== false ) ) {
			$appname = 'facebook';
		} elseif ( ( strpos( $_SERVER['REQUEST_URI'], 'openidcallback/windowslive' ) !== false ) ) {
			$appname = 'windowslive';
		} elseif ( ( strpos( $_SERVER['REQUEST_URI'], 'openidcallback/vkontakte' ) !== false ) ) {
			$appname = 'vkontakte';
		} elseif ( ( strpos( $_SERVER['REQUEST_URI'], 'openidcallback/linkedin' ) !== false ) ) {
			$appname = 'linkedin';
		} elseif ( ( strpos( $_SERVER['REQUEST_URI'], 'openidcallback/instagram' ) !== false ) || ( strpos( $_SERVER['REQUEST_URI'], 'openidcallback=instagram' ) !== false ) ) {
			$appname = 'instagram';
		} elseif ( ( strpos( $_SERVER['REQUEST_URI'], 'openidcallback/amazon' ) !== false ) || ( strpos( $_SERVER['REQUEST_URI'], 'openidcallback=amazon' ) !== false ) ) {
			$appname = 'amazon';
		} elseif ( ( strpos( $_SERVER['REQUEST_URI'], 'openidcallback/yahoo' ) !== false ) || ( strpos( $_SERVER['REQUEST_URI'], 'openidcallback=yaahoo' ) !== false ) ) {
			$appname = 'yahoo';
		} elseif ( ( strpos( $_SERVER['REQUEST_URI'], 'openidcallback/wordpress' ) !== false ) || ( strpos( $_SERVER['REQUEST_URI'], 'openidcallback=WordPress' ) !== false ) ) {
			$appname = 'WordPress';
		}
	} else {
		return;
...
function mo_openid_plugin_update() {
	...
	if ( $wpdb->get_var( "show tables like '$table_name'" ) != $table_name || empty( $time ) ) {
		...
		// phpcs:ignore WordPressVIPMinimum.Variables.RestrictedVariables.user_meta__wpdb__users -- should be safe, because we just identify the user and its not made public
...
function mo_openid_register_user() {
	...
	if ( mo_openid_check_empty_or_null( $_POST['email'] ) || mo_openid_check_empty_or_null( $_POST['password'] ) || mo_openid_check_empty_or_null( $_POST['confirmPassword'] ) ) {
		...
		return;
	} elseif ( strlen( $_POST['password'] ) < 6 || strlen( $_POST['confirmPassword'] ) < 6 ) {    // check password is of minimum length 6
		...
		return;
	} elseif ( strpbrk( $_POST['email'], $illegal ) ) {
		...
		return;
	} elseif ( strcmp( stripslashes( $_POST['password'] ), stripslashes( $_POST['confirmPassword'] ) ) != 0 ) {
