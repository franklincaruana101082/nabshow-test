...
class Preview_Data {
	 ...
	 *
	 * @return array
	 * @throws Invalid_Preview_Data When there's an error getting the preview data.
	 */
	 ...
	 * @return Action
	 *
	 * @throws Invalid_Preview_Data When there's an error with the preview data.
	 */
	static function generate_preview_action( $workflow_id, $action_number, $mode = 'preview' ) {
		...
		if ( ! $workflow || ! $action_number || ! is_array( $preview_data ) ) {
			throw Invalid_Preview_Data::generic();
		}
		...
		if ( ! $action || ! $action->can_be_previewed() ) {
			throw Invalid_Preview_Data::invalid_action();
		}
	 ...
	 *
	 * @return WC_Order
	 * @throws Invalid_Preview_Data When no preview order is found.
	 */
	protected static function get_preview_order( $offset = 0 ) {
		if ( $offset > 8 ) {
			throw Invalid_Preview_Data::order_not_found();
		}
