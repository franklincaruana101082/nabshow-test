...
class Event {
	...
	public function add_properties( WP_Post $event ) {
		$event->virtual                    = $this->is_event_virtual( $event );
	 ...
	 */
	protected function is_event_virtual( WP_Post $event ) {
	...
	protected function get_virtual_url( WP_Post $event ) {
		if ( ! $this->is_event_virtual( $event ) ) {
			return null;
		}
	...
	protected function get_virtual_linked_button( WP_Post $event ) {
		if ( ! $this->is_event_new_virtual( $event ) ) {
			return true;
		}
	...
	protected function get_virtual_show_embed_at( WP_Post $event ) {
		...
		if ( ! $this->is_event_new_virtual( $event ) ) {
			return Event_Meta::$value_show_embed_now;
		}

		$value = get_post_meta( $event->ID, Event_Meta::$key_show_embed_at, true );

		// If the metadata hasn't been set yet we want to default to "now".
		if ( empty( $value ) ) {
			return Event_Meta::$value_show_embed_now;
		}
	...
	protected function get_virtual_rsvp_email_link( $event ) {
		...
		// If the metadata hasn't been set yet we want to default to true.
		$link = $default || get_post_meta( $event->ID, Event_Meta::$key_rsvp_email_link, true );
		 ...
		 */
		$link = apply_filters( 'tribe_events_virtual_email_link', $link, 'rsvp' );
	...
	protected function get_virtual_ticket_email_link( $event ) {
		...
		// If the metadata hasn't been set yet we want to default to true.
		$link = $default || get_post_meta( $event->ID, Event_Meta::$key_ticket_email_link, true );
		 ...
		 * @param string  $type Ticket or RSVP.
		 */
		$link = apply_filters( 'tribe_events_virtual_email_link', $link, 'ticket' );
	 ...
	 */
	protected function is_event_new_virtual( $event ) {
		return $this->is_event_virtual( $event ) || tribe_context()->is_new_post();
	}
}
