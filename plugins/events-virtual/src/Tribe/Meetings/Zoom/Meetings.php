<?php
/**
 * Handles the creation and updates of Zoom Meetings via the Zoom API.
 *
 * @since   1.0.0
 *
...
class Meetings {
	...
	public function ajax_create( $nonce = null ) {
		if ( ! $this->check_ajax_nonce( self::$create_action, $nonce ) ) {
			return false;
		}
		 ...
		 */
		$force = apply_filters( 'tribe_events_virtual_meetings_zoom_meeting_force_recreate', true, $post_id );
		 ...
		 */
		$body = apply_filters( 'tribe_events_virtual_meetings_zoom_meeting_request_body', $body, $event, $this );
		...
		if ( empty( $post_id ) ) {
			...
			wp_send_json_error(
				[
					...
					'message' => _x( 'The post ID is missing from the request.', 'Ajax error message.',
						'events-virtual' )
				],
				400
			);
	...
	protected function prepare_meeting_data( $body ) {
		 ...
		 */
		$data = apply_filters( 'tribe_events_virtual_meetings_zoom_meeting_data', $data, $body );
	...
	public function ajax_remove( $nonce = null ) {
		if ( ! $this->check_ajax_nonce( self::$remove_action, $nonce ) ) {
			return false;
		}
