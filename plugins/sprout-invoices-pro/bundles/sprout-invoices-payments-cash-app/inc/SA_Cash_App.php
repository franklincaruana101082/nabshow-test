 ...
 */
class SA_Cash_App extends SI_Offsite_Processors {
	const MODE_LIVE = 'production';
	const CASHTAGOPTION = 'si_sc_cashtag';
	const PAYMENT_METHOD = 'Cash App';
	...
	public static function register() {
		self::add_payment_processor( __CLASS__, __( 'Cash App' , 'sprout-invoices' ) );
	}


	public static function public_name() {
		return __( 'Cash App' , 'sprout-invoices' );
	}

	public static function checkout_options() {
		$option = array(
			'icons' => array(
				SI_URL . '/resources/front-end/img/CashApp.png',
				),
			'label' => __( 'Cash App' , 'sprout-invoices' ),
	...
	public static function register_settings( $settings = array() ) {
		...
		$settings['payments'] = array(
			'si_cash_app_settings' => array(
				'title' => __( 'Cash App' , 'sprout-invoices' ),
				'weight' => 200,
				'settings' => array(
	...
	public function send_offsite( SI_Checkouts $checkout ) {
		...
		wp_redirect( 'https://cash.app/' . self::$cashtag . '/' . (float) $payment_amount );
		exit();
	}
