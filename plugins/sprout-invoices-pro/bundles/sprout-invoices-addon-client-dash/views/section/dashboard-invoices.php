<table id="si_client_dashboard_invoices" class="table table-hover">
				...
				<tr class="info_row">
					...
					<td colspan="3">
						...
						<div class="estimate_info_row_wrap">
							<?php if ( $estimate_id = $invoice->get_estimate_id() ) : ?>
								<?php if ( get_post_type( $estimate_id ) === SI_Estimate::POST_TYPE ) : ?>
									<small><?php printf( '<b>%s:</b> <a href="%s">%s</a>', __( 'Estimate', 'sprout-invoices' ), get_permalink( $estimate_id ), get_the_title( $estimate_id ) ) ?></small>
								<?php endif ?>
							<?php endif ?>
						</div>
					</td>
							...
							<?php foreach ( $inv_payments as $payment_id ) : ?>
								<?php
									...
									$method_name = apply_filters( 'si_display_payment_name', $method, $payment );
										?>
								<small><?php printf( __( '<b>%s:</b> %s', 'sprout-invoices' ), $method_name, sa_get_formatted_money( $payment->get_amount(), $invoice_id ) ) ?></small>
							<?php endforeach ?>
						<?php endif ?>
					</td>
	...
	<?php if ( count( $invoices ) > 1 && $total_balance_due ) : ?>
		<tfoot>
			<tr>
				<td colspan="3">&nbsp;</td>
				<td colspan="1"><?php printf( __( 'Balance Due: <b>%s</b>', 'sprout-invoices' ), sa_get_formatted_money( $total_balance_due, get_the_id() ) ) ?></td>
			</tr>
		</tfoot>
	<?php endif ?>
</table>
