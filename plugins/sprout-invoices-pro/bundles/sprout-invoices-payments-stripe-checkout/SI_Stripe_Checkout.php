...
class SI_Stripe_Checkout extends SI_Offsite_Processors {
	...
	public static function register() {
		...
		self::register_query_var( self::PAYMENT_SLUG, array( __CLASS__, 'maybe_back_from_stripe' ) );

		add_action( 'si_settings_saved', array( get_class(), 'maybe_create_hook_endpoint' ) );

		self::register_query_var( self::API_QUERY_VAR, array( __CLASS__, 'webhook_listener' ) );
	...
	protected function __construct() {
		...
		self::$cancel_url = get_option( self::CANCEL_URL_OPTION, add_query_arg( array( 'stripe_checkout' => 'cancelled' ), home_url( '/' ) ) );
		if ( self::$cancel_url === '' ) {
			$url = add_query_arg( array( 'stripe_checkout' => 'cancelled' ), home_url( '/' ) );
	...
	public static function payment_button( $invoice_id = 0 ) {
		...
		$view = self::load_addon_view( 'button', array(
				'pubkey' => $pub_key,
				'session_id' => $session,
		), true );
		print $view;
	...
	public static function create_session( $invoice_id = 0 ) {
		...
		if ( si_is_invoice_recurring( $invoice ) ) {
			...
			$create = array(
				...
				'success_url' => add_query_arg( array( self::PAYMENT_SLUG => $payment_id ), get_permalink( $invoice_id ) ),
				'cancel_url' => add_query_arg( array( self::PAYMENT_SLUG => $payment_id, 'cancelled' => 'true' ), get_permalink( $invoice_id ) ),
			);
	...
	public static function maybe_back_from_stripe() {
		...
		if ( isset( $_GET['cancelled'] ) && 'true' == $_GET['cancelled']  ) {
			...
			wp_redirect( remove_query_arg( array( self::PAYMENT_SLUG, 'cancelled' ) ) );
			exit();
	...
	public static function get_webhook_url() {
		return add_query_arg( array( self::API_QUERY_VAR => 'callback' ), home_url() );
	}
