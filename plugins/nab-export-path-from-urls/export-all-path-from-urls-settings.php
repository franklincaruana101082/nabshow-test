		...
		if (isset($_POST['export']))
		{
			...
			if (isset($_REQUEST['_wpnonce']))
			{
				...
				if (!empty($_POST['post-type']) && !empty($_POST['export-type']) && !empty($_POST['additional-data']) && !empty($_POST['post-status']) && !empty($_POST['post-author']) && !empty($_POST['number-of-posts']))
				{
					...
					$selected_post_type = $this->eau_get_selected_post_type($post_type, $custom_posts_names);

					$this->eau_generate_output($selected_post_type, $post_status, $post_author, $remove_woo_attributes, $exclude_domain, $post_per_page, $offset, $export_type, $additional_data, $csv_name, $posts_from, $posts_upto);
