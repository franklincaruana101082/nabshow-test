		...
		public function ep_ajax_zoom_link_creator() {
			...
			if ( empty( $zoom_id_from_content_meta ) || empty( $zoom_id_from_content_meta[0] ) ) {
				if ( current_user_can( 'administrator' ) ) {
					$message = "<p class='error'>Hi Admin, please add zoom id in the edit screen of this page.</p>";
					$code = 402;
				} // else keep the return blank.

			} else if ( empty( $associate_products ) ) {
				if ( current_user_can( 'administrator' ) ) {
					$message = "<p class='error'>Hi Admin, please assign associated products to the content.</p>";
					$code = 402;
				} // else keep the return blank.
				...
				if ( ! is_user_logged_in() ) {
					$message = "<p class='error'>Please log in to see your purchased zoom link.</p>";
					$code = 403;
				} else {
					// User logged in.
					...
					if ( ! empty( $zoom_unique_url ) ) {
						$message = "<a href='$zoom_unique_url' class='button' id='join-zooom-button' target='_blank'>Join Meeting</a>";
						$code = 200;
					} else {
						...
						$message = "<p class='error'>The meeting link is not available, please purchases the content or contact administrator.</p>";
					}
				}
		...
		public function ep_zoom_shortcode() {
			...
			if ( isset( $zoom_id_from_content_meta[0] ) && ! empty( $zoom_id_from_content_meta[0] ) ) {
				...
				if( ! empty( $zoom_unique_url ) ) {
					...
					echo "<p class='error check-in-deep'>The meeting link is not available, please purchases the content or contact administrator.</p>";
				}
