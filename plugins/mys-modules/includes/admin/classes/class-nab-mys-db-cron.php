	...
	class NAB_MYS_DB_CRON {
		...
		public function nab_mys_cron_master_tracks( $data_to_migrate ) {
			...
			foreach ( $data as $track ) {
				...
				$term_id = $this->nab_mys_cron_get_wpid_from_meta( 'tracks', 'trackid', $trackid, 'taxonomy' );
		...
		public function nab_mys_cron_get_wpid_from_meta( $type_name, $main_mys_key, $main_mys_value, $type = 'post_type' ) {
			...
			if ( 'post_type' === $type ) {
				...
				$wp_id = isset( $wp_post->posts[0]->ID ) ? $wp_post->posts[0]->ID : '';
				...
				$wp_id = isset( $terms[0]->term_id ) ? $terms[0]->term_id : '';
			}
