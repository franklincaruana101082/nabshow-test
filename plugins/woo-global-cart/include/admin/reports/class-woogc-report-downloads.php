 ...
 * WC_Report_Downloads.
 */
class WooGC_Report_Downloads extends WP_List_Table {
	...
	public function get_items( $current_page, $per_page ) {
        ...
        foreach ( $network_sites as $site ) 
            {
		        ...
		        if ( ! is_null( $filters->product_id ) || ! is_null( $filters->download_id ) || ! is_null( $filters->order_id ) ) {
                    ...
                    $select_fields  =   'download_log_id ,   timestamp ,   permission_id ,   user_id,    user_ip_address,    permission_id,    download_id,    product_id,    order_id ,   order_key ,   user_email ,   user_id ,   downloads_remaining,    access_granted  ,  access_expires ,   download_count';
		        }
                ...
                $query[]  = "SELECT " . $select_fields  .", ".  $blog_id ." as blog_id {$query_from}";
		...
		$this->max_items = $wpdb->get_var( "SELECT COUNT( download_log_id ) FROM ( " . implode( " UNION ALL ", $query ) . ") as q " ); // WPCS: cache ok, db call ok, unprepared SQL ok.
	}
}
