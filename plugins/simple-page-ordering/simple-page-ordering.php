...
if ( ! class_exists( 'Simple_Page_Ordering' ) ) :
	...
	class Simple_Page_Ordering {
		...
		public static function ajax_simple_page_ordering() {
			...
			$previd = filter_input( INPUT_POST, 'previd', FILTER_SANITIZE_NUMBER_INT );
			$nextid = filter_input( INPUT_POST, 'nextid', FILTER_SANITIZE_NUMBER_INT );

			// check and make sure we have what we need
			if ( empty( $id ) || ( empty( $previd ) && empty( $nextid ) ) ) {
				die( - 1 );
			}
			...
			$excluded = filter_input( INPUT_POST, 'excluded', FILTER_SANITIZE_STRING );

			$previd   = empty( $previd ) ? false : (int) $previd;
			$nextid   = empty( $nextid ) ? false : (int) $nextid;
			$start    = empty( $start ) ? 1 : (int) $start;
			$excluded = empty( $excluded ) ? array( $post->ID ) : array_filter( (array) json_decode( $excluded ), 'intval' );
