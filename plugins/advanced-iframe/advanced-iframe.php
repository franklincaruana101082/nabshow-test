...
if (!class_exists('advancediFrame')) {
    class advancediFrame
    {
        ...
        function aiCheckIframes() {
            ...
            if ($options['check_iframe_cronjob'] === 'true') {
               ...
               if ($result['overall_status'] === 'red') {
                  ...
                  $body .= __('At least one of the iframes you have configured has a src which does not work properly.<br>Please go to the administration of advanced iframe pro to get more details.', 'advanced-iframe');
               } else if ($result['overall_status'] === 'orange') {
                  $subject = __('At least one of your iframes is not optimal configured!', 'advanced-iframe');
                  $body .= __('At least one of the iframes you have configured has most likely an src which is redirected.<br>Please go to the administration of advanced iframe pro to get more details.', 'advanced-iframe');
               } else {
        ...
        function iframe_defaults() {
            ...
            $iframeAdminOptions = array(
                ...
                'change_iframe_links_href' => '' , 'delete_options_db' => 'false',
				'inline_config_file' => 'none', 'replace_iframe_tags' => 'false',
				'check_iframe_batch_size' => 100, 'check_iframe_cronjob_email_always' => 'true',
  ...
  function my_the_content( $more_link_text = null, $strip_teaser = false) {
    ...
    function aip_close_message_permanent() {
	     ...
	     $id = $_POST['id'];
		 $options = get_option('advancediFrameAdminOptions');
		 // get existing settings and add/update the existing...
	     $closed_messages = $options['closed_messages'];
		 if (!isset($closed_messages)) {
...
function advanced_iframe_plugin_meta_pro($links, $file)
{
  ...
  function render_ai_gutenberg( $attributes ) {
      ...
      $add_array = (isset($attributes['additional'])) ?  shortcode_parse_atts($attributes['additional']) : array();
      $new_attributes = array_merge ($add_array, $attributes);
      unset($new_attributes['additional']); 
      return $cons_advancediFrame->do_iframe_script($new_attributes, null);
  }
